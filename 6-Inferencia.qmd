...
Sejam $\boldsymbol{\hat{\mu}}=\bar{X} \text { e } \hat{\boldsymbol\Sigma}=S=\frac{1}{n-1} \sum_{j=1}^n(\boldsymbol{X}-\boldsymbol{X})(\boldsymbol{X}-\boldsymbol{X})^{\top}$.
...
Obs: A distribuição Wishart é uma generalização da distribuição Gama, e é definida como a soma de produtos de normais multivariadas independentes de média $\boldsymbol{0}$ e variância $\boldsymbol\Sigma$.
